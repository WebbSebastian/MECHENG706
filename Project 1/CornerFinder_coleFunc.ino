...
float IrYawError(bool range)//Function calculates the yaw error in degrees of the robot based on the IR sensors
{
  ...
  /*  
  if (error > 360){
    error -= (360 * ((int)error/360))
  }
  else if (error < 0){
    error += (360 * ((int)abs(error)/360) + 1))
  }
  */
  return error;
}
...
float noiseCapped(float input, float inputPrev, float threshold) {

  if (abs(input - inputPrev) > threshold){
    input = inputPrev;        
  }
...
{
  float angleError = 5 * yaw_angle;
  float edgeError = 10 * (edgeDist - distance);

  left_font_motor.writeMicroseconds(SpeedCapped(angleError + edgeError));
  left_rear_motor.writeMicroseconds(SpeedCapped(angleError - edgeError));
  right_rear_motor.writeMicroseconds(SpeedCapped(-angleError + edgeError));
  right_font_motor.writeMicroseconds(SpeedCapped(-angleError - edgeError));
...
void FindCorner(float sonarDistance, float shortIR, float yawAngle){
  ...
  static float timeDebug = millis();

  float timeExitInitial = 0;
  
  float time90deg = 2800.0 * (100.0/(float)speed_val);

  //used for case 0, taken outside loop so can be adjusted in case 4
