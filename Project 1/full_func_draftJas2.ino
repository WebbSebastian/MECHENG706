...
void loop(void) //main loop
{
  ...
  if(shortRun)
  {
    driveAtDistFromWall(shortFront, shortRear, wallDist, 5);
    if(distanceUS <= 8)
    {
      shortRun = 0
      drive = 1;
  ...
  if(drive == 1)
  {
    if(driveDir)
    {
      driveAtDistFromWall(longFront, longRear, wallDist, 5);
      if(distanceUS <= 8)
      {
        drive = 0;
        driveDir = 0;
    ...
    {
      driveAtDistFromWall(longFront, longRear, wallDist, 170);
      if(distanceUS >= 167)
      {
        drive = 0;
        driveDir = 1;
  ...
  if(edge)
  {
    if(edgeCalc)
    {
      wallDist = wallDist + (hasRotated * lateralStep);
      edgeCalc = 0;
    }
    if(wallDist >= (60))
    {
